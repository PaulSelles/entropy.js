...
(function(window) {
	...
	function SHA1() {
		...
		this._hashMessage = function (message) {
	    	return secureHash(convertToBlocks(preProcessMessage(message)));
		} 
	}		 

	// Load a message to be hased
	SHA1.prototype.loadMessage = function (message) {
	...
	SHA1.prototype.appendMessage = function (message) {
	...
	SHA1.prototype.clearMessage = function () {
		this.message = "";
	}

	// Hash passed message or hash stored message
	SHA1.prototype.hashMessage = function (message) {
		
		if (message === undefined)
			message = this.message;

		this.clearMessage();
	    return this._hashMessage(message);
	}
